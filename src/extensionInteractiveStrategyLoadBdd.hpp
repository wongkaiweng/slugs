#ifndef __EXTENSION_INTERACTIVE_STRATEGY_LOAD_BDD_HPP
#define __EXTENSION_INTERACTIVE_STRATEGY_LOAD_BDD_HPP
 ...
 *
 */
template<class T> class XInteractiveStrategyLoadBDD : public T {
protected:
    // Constructor
    XInteractiveStrategyLoadBDD<T>(std::list<std::string> &filenames) : T(filenames) {}
...
public:
    static GR1Context* makeInstance(std::list<std::string> &filename) {
        return new XInteractiveStrategyLoadBDD<T>(filename);
    }
    ...
    */
    void init(std::list<std::string> &filenames) {
        ...
        if (filenames.size()<2) {
            throw "Error: At least two file names are needed when using the supplied options! First: slugsin file, Second: bdd file";
        }
        ...
        */
            ...
            if ((currentLine.length()>0) && (currentLine[0]!='#')) {
                if (currentLine[0]=='[') {
                    if (currentLine=="[INPUT]") {
                        ...
                        std::cerr << "Found all inputs and outputs\n";
                        break;
                    }
                } else {
                    if (readMode==0) {
        ...
        std::cerr << bddFileName.c_str() << "\n"; 
        strategy = mgr.readBDDFromFile(bddFileName.c_str(),varsBDDread);
        std::cerr << "BDD read.";
